...
public class ConnectServer {
	 ...
	 * 画板端口
	 */
	private static ServerSocket padSocket;
	/**
	 * 键盘socket
	 */
	 ...
	 * 画板端口
	 */
	public final static int PC_PAD_CON = 8629;
	...
	private Runnable padRunnable = new Runnable() {

		@Override
		public void run() {
			while (true) {
				System.out.println("画板服务器正在等待连接...");
				Socket socket;
				try {
					socket = padSocket.accept();
					// 对应每一个请求，开启另外的线程去处理
					ThreadManager.getInstance().getExecutorService()
							.execute(new PadThread(socket));
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	public ConnectServer() throws IOException {
		...
		padSocket = new ServerSocket(PC_PAD_CON);
